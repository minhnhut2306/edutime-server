...
const findSubjectByNameFlexible = async (
  subjectName,
  schoolYearId,
  excludeId = null
) => {
  ...
  return allSubjects.find(
    (s) => normalizeSubjectName(s.name) === inputNormalized
  );
};
...
const deleteSubject = async (id) => {
  ...
  if (teacherCount > 0) {
    throw new Error(
      `Không thể xóa môn "${subject.name}" vì có ${teacherCount} giáo viên đang dạy môn này. Vui lòng cập nhật thông tin giáo viên trước.`
    );
  }

  const teachingRecordsCount = await TeachingRecords.countDocuments({
    subjectId: id,
  });
  if (teachingRecordsCount > 0) {
    throw new Error(
      `Không thể xóa môn "${subject.name}" vì có ${teachingRecordsCount} bản ghi giảng dạy liên quan. Vui lòng xóa các bản ghi giảng dạy trước.`
    );
  }
